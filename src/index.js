...
//this used to work correctly but stop when the links were non existant
const puppeteer = require('puppeteer');
const axios = require('axios');
...
// Function to check if URL belongs to target domain (browser context)
function isTargetDomain(url, baseDomain) {
    try {
        const urlObj = new URL(url);
        const domain = urlObj.hostname.replace('www.', '');
        const targetDomain = new URL(baseDomain).hostname.replace('www.', '');
...
console.log(websiteId); // Outputs: maitridesigns-com-username
...
async function extractLinks(page) {
    return page.evaluate((baseDomain) => {
        // Redefine the isTargetDomain function inside the evaluate
        ...
        const links = Array.from(document.querySelectorAll('a[href]'))
                        .map(a => new URL(a.href, document.baseURI).href)
                        .filter(href => isTargetDomain(href) && href.startsWith(baseDomain));
        return links;
    }, targetUrl);
}
...
async function scrapeWebMap(currentUrl, depth) {
    ...
    try {
        ...
        const pageData = await page.evaluate((baseDomain) => {
            // Redefine the isTargetDomain function inside evaluate
            function isTargetDomain(url) {
                try {
                    const urlObj = new URL(url);
                    const domain = urlObj.hostname.replace('www.', '');
            ...
            const links = Array.from(document.querySelectorAll('a[href]'))
                            .map(a => new URL(a.href, document.baseURI).href)
                            .filter(href => isTargetDomain(href) && href.startsWith('http') && !href.includes('#') && !/\.(pdf|jpg|jpeg|png|gif|svg|js|css)$/i.test(href));
            return { title, links };
        }, targetUrl);
